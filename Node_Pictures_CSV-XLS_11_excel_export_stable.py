...
class App(tk.Tk):
    ...
    def init_tab_process(self):
        ...
        self.format_dialog = FilenameFormatDialog(
            ...
            on_update_export_data=self.update_export_data   # <-- pass the callback here
        )
        ...
        csv_frame = ttk.Frame(btn_frame)
        csv_frame.pack(side="left", fill="y", expand=True, padx=(0, 12))

        excel_frame = ttk.Frame(btn_frame)
        excel_frame.pack(side="left", fill="y", expand=True)

        # --- CSV Export Section ---
        self.csv_columns_vars = {}  # {col: tk.BooleanVar}
        self.csv_check_frame = ttk.LabelFrame(csv_frame, text="Columns to export (CSV)")
    ...
    ttk.Button(csv_frame, text="Export CSV", command=self.export_csv).pack(side="bottom", pady=(4, 0))
    ...
    def update_excel_mapping_ui(self):
        ...
        if not self.excel_file_chosen or not self.excel_cols:
            return
        # File name label
        ttk.Label(self.excel_mapping_frame, text=self.excel_filename_var.get(), foreground="blue").pack(anchor="w", pady=2)
    ...
    self.excel_mapping_frame = ttk.Frame(excel_frame)
    self.excel_mapping_frame.pack(fill="both", expand=True, padx=3, pady=3)

    excel_btns_frame = ttk.Frame(excel_frame)
    ...
    def update_existing_excel_from_ui(self):
        ...
        try:
            ...
            num_rows_to_update = min(len(self.export_df), len(data_rows))

            for row_idx in range(num_rows_to_update):
