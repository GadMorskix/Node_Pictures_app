...
class App(tk.Tk):
    ...
    def init_tab_process(self):
        ...
        self.export_tree_frame = ttk.Frame(self.export_frame)
        self.export_tree_frame.pack(fill="both", expand=True)

        self.export_tree = ttk.Treeview(self.export_tree_frame, show="headings")
        self.export_tree.pack(side="left", fill="both", expand=True)

        self.export_vsb = ttk.Scrollbar(self.export_tree_frame, orient="vertical", command=self.export_tree.yview)
        self.export_vsb.pack(side="right", fill="y")

        self.export_hsb = ttk.Scrollbar(self.export_frame, orient="horizontal", command=self.export_tree.xview)
        self.export_hsb.pack(side="bottom", fill="x")

        self.export_tree.configure(yscrollcommand=self.export_vsb.set, xscrollcommand=self.export_hsb.set)
    ...
    def show_export_df(self):
        # Clear current contents
        for col in self.export_tree.get_children():
            self.export_tree.delete(col)
        columns = list(self.export_df.columns)
        self.export_tree["columns"] = columns
        self.export_tree["show"] = "headings"
        
        # Set column widths
        for col in columns:
            maxlen = len(str(col))
            for val in self.export_df[col]:
                maxlen = max(maxlen, len(str(val)))
            width = max(100, min(300, maxlen * 12 + 16))
            self.export_tree.heading(col, text=col)
            self.export_tree.column(col, width=width, minwidth=80, anchor="center")

        # Insert rows
        for _, row in self.export_df.iterrows():
            self.export_tree.insert("", "end", values=list(row))
